...
複数のサーバー上でコンテナを自動的に管理・運用するための**オーケストレーションプラットフォーム**。Dockerがコンテナを「動かす」ツールなのに対し、Kubernetesはそのコンテナを「どこで・何台・どう動かすか」を管理する。

- 複数のサーバー（ノード）をまとめてクラスタとして管理する
- コンテナが落ちたら自動で再起動する
- トラフィックが増えたら自動でコンテナ数を増やす（オートスケール）
- 新バージョンへの切り替えやロールバックを自動化できる

**Podとは**

KubernetesにおけるデプロイとスケールアウトはPod単位で行われる。  
1つのPodは1つ以上のコンテナをまとめたもので、同じPod内のコンテナはネットワーク・ストレージを共有する。
...
新バージョンをデプロイするとき、古いバージョンのサーバープロセス（Goで書いたAPIサーバーなど）を停止する必要がある。このとき「プロセスの停止」をどう行うかの設計の話。PCのシャットダウンとは別の話で、**サーバー上で動いているプログラムの終了**を指す。

**SIGTERMとは**

Unix/Linuxでプロセスに「終了してください」と伝えるシグナル（信号）のこと。KubernetesやDockerがコンテナを止めるとき、まずSIGTERMを送る。プロセスはこのシグナルを受け取って後処理をしてから終了できる。

```
通常の停止（グレースフルなし）
SIGTERM受信 → 即プロセス終了 → 処理中リクエストが途中で切断される → クライアントにエラー
...
デプロイのたびにユーザーへのエラーが発生しないようにするため。特に処理時間が長いAPIやファイルアップロードなどは、途中で切断されるとユーザーへの影響が大きい。
...
defer cancel()
server.Shutdown(ctx)  // 30秒以内に処理中リクエストが完了するのを待って終了
```

ロードバランサがラメダックとして認識する時間を確保することも必要。

**他の言語・フレームワークでの扱い**

Goはシンプルな標準ライブラリのHTTPサーバーを自分で動かすため、グレースフルシャットダウンを自前で実装する必要がある。一方、他の言語ではAPサーバー（アプリケーションサーバー）がこの処理を担うことが多い。

| 言語・環境 | APサーバー | グレースフルシャットダウン |
|------------|-----------|--------------------------|
| Java | Tomcat / Jetty | Spring Boot 2.3以降は設定1行（`server.shutdown=graceful`）で有効 |
| Ruby | Puma | デフォルトでSIGTERMに対応、処理中リクエストを待つ |
| Python | Gunicorn | SIGTERM受信時に処理中ワーカーの完了を待って終了 |
| Node.js | （なし） | Goと同様に自前実装が必要。Fastifyなどのフレームワークが補助機能を提供 |
| Go | （なし） | 自前実装が必要（標準ライブラリの `server.Shutdown()` を使う） |

APサーバーを使う構成では、このような「プロセス管理」の責務をAPサーバーが肩代わりするため、アプリのコードに書く必要がなくなる。
