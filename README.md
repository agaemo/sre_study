...
> **注意：** このリポジトリは個人学習用に Claude Code を使って作成したものです。内容の正確性は完全には保証されていません。SRE本など一次情報と照らし合わせながら学習することを推奨します。
...
3つのフェーズを通じて、SREの核心にある問いに向き合ってきた。

- **フェーズ1**：「今、システムに何が起きているか」を数値で答えられるか
- **フェーズ2**：「障害が起きたとき、どこまで連鎖を止められるか」を設計できるか
- **フェーズ3**：「変更を安全に届けながら、システムを前に進められるか」を実践できるか

SREが目指すのは、信頼性の最大化ではない。ビジネスが受け入れられるリスクのレベルを定め、そのなかで開発速度と安定性を両立させることだ。エラーバジェットはその「余白」を可視化し、チームが感覚ではなく数値で判断できるようにする。

障害は起きる。問題はそれをどう検知し、どう止め、どう学ぶかだ。ポストモーテムは犯人を探すためではなく、「なぜシステムがそれを防げなかったか」を問うためにある。

コードを書くとき、デプロイするとき、設計を決めるとき——SREの視点は特別な役職の人間だけのものではない。「このシステムは壊れたときにどう振る舞うか」を常に問い続けることが、SWEとSREを統合するということだと思う。
